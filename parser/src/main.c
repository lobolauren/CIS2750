...
int main(int argc, char **argv) { 
    //hi comment
    GPXdoc * docs = createValidGPXdoc(argv[1], "gpx.xsd");
    //printf("val : %d\n", validateGPXDoc(docs, "gpx.xsd"));
    GPXdocToString(docs); 
    //createNewRoute("{\"lat\":44.69708,\"lon\":-124.653634}|{\"lat\":44.71238,\"lon\":-125.653634}","{\"name\":\"name of route\"}",argv[1],2);
  //Route *r = getRoute(docs,">GHTC James St-Victoria Road");
  //  char*json = gpxDataToJSONTracks(argv[1], "Memorial Forest Loop");
  //routeDataToJSON( argv[1],  44.596100, -123.620570, 44.597150, -123.620220, 10);
  //  printf("%s\n",json);
     routeDataToJSON(argv[1],  43.53729,-80.218267, 43.531367, -80.226122, 10);
...
int main(int argc, char **argv) { 
    ...
    printf("\n6. Testing numRoutesWithLength with '160' and delta '11'...\n");
    int numRoutes = numRoutesWithLength(doc, 150.0, 11.0);
    printf("\t%d routes with length 150 += 11.\n", numRoutes);

    printf("\n7. Testing numRoutesWithLength with '160' and delta '10'...\n");
    numRoutes = numRoutesWithLength(doc, 150.0, 10.0);
    printf("\t%d routes with length 150 += 11.\n", numRoutes);

    printf("\n8. Testing numTracksWithLength with '330' and delta '8'...\n");
    int numTracks = numTracksWithLength(doc, 330.0, 8.0);
    printf("\t%d routes with length 330 += 8.\n", numTracks);

    printf("\n9. Testing numTracksWithLength with '330' and delta '7'...\n");
    numTracks = numTracksWithLength(doc, 330.0, 7.0);
    printf("\t%d routes with length 330 += 7.\n", numTracks);
